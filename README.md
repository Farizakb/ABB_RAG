...
| DB browser (pgAdmin) | `make db-ui` | `docker compose --profile tools up -d --wait pgadmin`, then open `http://127.0.0.1:5050` — no login, database pre-registered (DB password: `POSTGRES_PASSWORD`, default `abb`) |

...
# convention as backend/rag/conftest.py.
"""Fixtures shared by every backend/chat test.

`db` builds and migrates a dedicated `*_chat_test` database from the real
migration file, the same approach backend/rag/conftest.py uses for its own
`*_test` database -- a different suffix so a concurrent `pytest
backend/rag` run never truncates tables this suite depends on, and vice
versa.
...
def _reachable(url: str) -> str:
    ...
    this rationale in backend/rag/conftest.py: `settings.database_url`
